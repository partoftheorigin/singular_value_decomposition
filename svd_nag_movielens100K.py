...
def matrix_factorization(R, P, Q, K, steps=1000, alpha=0.001, gamma=0.9):
    ...
    for step in range(steps):
        for i in range(len(R)):
            for j in range(len(R[i])):
                if R[i][j] > 0:
                    ...
                    for k in range(K):
                        xp = P[i][k] - gamma * PW[i][k]
                        xq = Q[k][j] - gamma * QW[k][j]
                        PW[i][k] = gamma * PW[i][k] + alpha * (2 * eij * xq)
                        P[i][k] = P[i][k] + PW[i][k]
                        QW[k][j] = gamma * QW[k][j] + alpha * (2 * eij * xp)
                        Q[k][j] = Q[k][j] + QW[k][j]
        eR = np.dot(P, Q)
        e = 0
        for i in range(len(R)):
        ...
        if e < 0.001:
            break
        l1 = ['nag', K, step+1, e]
        with open(r'/home/linux/Dropbox/Curioso/phd-work/recommendation-systems/curioso/data/svd_step_error_results_movielens100K_943x1682_nag.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(l1)
        if (step+1) % 100 == 0:
            X_pred = np.dot(P, Q)
            ...
            l = ['nag', n_users, n_items, K, step+1, e, rmse_training, rmse_testing, mae_training, mae_testing]

            with open(r'/home/linux/Dropbox/Curioso/phd-work/recommendation-systems/curioso/data/svd_results_movielens100K_943x1682_nag.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerow(l)

def rmse(prediction, actual):
	return sqrt(((prediction - actual) ** 2).mean())
