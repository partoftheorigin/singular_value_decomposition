...
def matrix_factorization(R, P, Q, K, steps=1000, alpha=0.001, beta=0.002):
    Q = Q.T
    ...
    for step in range(steps):
        ...
        if e < 0.001:
            break
        l1 = ['new_regularization', K, step+1, e]
        with open(r'/home/linux/Dropbox/Curioso/phd-work/recommendation-systems/curioso/data/svd_step_error_results_movielens100K_943x1682_new_regularization.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(l1)
        if (step+1) % 100 == 0:
            X_pred = np.dot(P, Q)
            ...
            l = ['new_regularization', n_users, n_items, K, step+1, e, rmse_training, rmse_testing, mae_training, mae_testing]

            with open(r'/home/linux/Dropbox/Curioso/phd-work/recommendation-systems/curioso/data/svd_results_movielens100K_943x1682_new_regularization.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerow(l)
